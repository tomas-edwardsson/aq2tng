...
qboolean
Pickup_Special (edict_t * ent, edict_t * other)
{
...
void
Drop_Special (edict_t * ent, gitem_t * item)
{
...
/*      if ( stricmp( item->pickup_name, LASER_NAME ) == 0 
  ...
  if (stricmp (item->pickup_name, BAND_NAME) == 0
      && ent->client->pers.inventory[ITEM_INDEX (item)] <= 1)
    {
      ...
      if (ent->client->pers.inventory[ITEM_INDEX (FindItem ("Pistol Clip"))] >
	  2)
	ent->client->pers.inventory[ITEM_INDEX (FindItem ("Pistol Clip"))] =
	  2;
      ent->client->pers.max_shells = 14;
      if (ent->client->pers.
	  inventory[ITEM_INDEX (FindItem ("12 Gauge Shells"))] > 14)
	ent->client->pers.
	  inventory[ITEM_INDEX (FindItem ("12 Gauge Shells"))] = 14;
	...
	ent->client->pers.
      ...
      if (ent->client->pers.inventory[ITEM_INDEX (FindItem (KNIFE_NAME))] >
	  10)
	ent->client->pers.inventory[ITEM_INDEX (FindItem (KNIFE_NAME))] = 10;

      ent->client->pers.max_slugs = 20;
      if (ent->client->pers.
	  inventory[ITEM_INDEX (FindItem ("AP Sniper Ammo"))] > 20)
	ent->client->pers.
	  inventory[ITEM_INDEX (FindItem ("AP Sniper Ammo"))] = 20;

      if (ent->client->unique_weapon_total > unique_weapons->value
	  && !allweapon->value)
	{
	  DropExtraSpecial (ent);
	  gi.cprintf (ent, PRINT_HIGH,
		      "One of your guns is dropped with the bandolier.\n");
	}
    }
  Drop_Spec (ent, item);
...
// this is the order I'd probably want to drop them in...       
  ...
  else if (ent->client->pers.inventory[ITEM_INDEX (FindItem (HELM_NAME))])
    Drop_Special (ent, FindItem (HELM_NAME));
...
*/
void
Touch_Item (edict_t * ent, edict_t * other, cplane_t * plane,
	    csurface_t * surf)
{
  qboolean taken;
